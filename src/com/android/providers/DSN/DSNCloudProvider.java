 ...
 * .
 */
public class DSNCloudProvider extends FileSystemProvider {
    private static final String TAG = "DecenterCloudProvider";
    private static final boolean DEBUG = false;

    private static final String AUTHORITY = Constants.CLOUD_AUTHORITY;
    private static final String DOC_ID_ROOT = Constants.CLOUD_ROOT_ID;
    ...
    public boolean onCreate() {
        ...
        Log.d("XULU_LOG","DecenterCloudProvider onCreate");
    ...
    public Cursor queryRoots(String[] projection) throws FileNotFoundException {
        ...
        // subsequent queries.
        Log.d("XULU_LOG","DecenterCloudProvider queryRoots");
        ...
        row.add(Root.COLUMN_ICON, R.mipmap.ic_laucher_dsnremote);
        row.add(Root.COLUMN_TITLE, getContext().getString(R.string.root_cloud));
    ...
    public Path findDocumentPath(String parentDocId, String docId) throws FileNotFoundException {
        Log.d("XULU_LOG","DecenterCloudProvider findDocumentPath");
    ...
    public String createDocument(String parentDocId, String mimeType, String displayName)
        ...
        Log.d("XULU_LOG","DecenterCloudProvider createDocument");
    ...
    public void deleteDocument(String docId) throws FileNotFoundException {
        // Delegate to real provider
         Log.d("XULU_LOG","DecenterCloudProvider deleteDocument");
        final long token = Binder.clearCallingIdentity();
        try {
    ...
    public String renameDocument(String docId, String displayName)
            throws FileNotFoundException {
                   Log.d("XULU_LOG","DecenterCloudProvider renameDocument");
        final long token = Binder.clearCallingIdentity();
    ...
    public Cursor queryDocument(String docId, String[] projection) throws FileNotFoundException {
        // Delegate to real provider
               Log.d("XULU_LOG","DecenterCloudProvider queryDocument" + docId);
        final long token = Binder.clearCallingIdentity();
        Cursor cursor = null;
        try {
            if (RawDocumentsHelper.isRawDocId(docId)) {
            ...
            if (DOC_ID_ROOT.equals(docId)) {
                Log.d("XULU_LOG","DecenterCloudProvider queryDocument 1");
                includeDefaultDocument(result);
            } else {
                Log.d("XULU_LOG","DecenterCloudProvider queryDocument 2");
    ...
    public Cursor queryChildDocuments(String parentDocId, String[] projection, String sortOrder)
            throws FileNotFoundException {
        Log.d("XULU_LOG","DecenterCloudProvider queryChildDocuments Override");
    ...
    public Cursor queryChildDocumentsForManage(
            ...
            throws FileNotFoundException {
        Log.d("XULU_LOG","DecenterCloudProvider queryChildDocumentsForManage");
    ...
    private Cursor queryChildDocuments(String parentDocId, String[] projection,
        ...
        Log.d("XULU_LOG","DecenterCloudProvider queryChildDocuments a" + parentDocId);
        ...
        try {
            if (RawDocumentsHelper.isRawDocId(parentDocId)) {
                Log.d("XULU_LOG","DecenterCloudProvider queryChildDocuments a isRawDocId");
                return super.queryChildDocuments(parentDocId, projection, sortOrder);
            }

            assert (DOC_ID_ROOT.equals(parentDocId));
            Log.d("XULU_LOG","DecenterCloudProvider queryChildDocuments a isRawDocId1");
            final DownloadsCursor result = new DownloadsCursor(projection,
                    getContext().getContentResolver());
            /*
            if (manage) {
            ...
            */
            Log.d("XULU_LOG","DecenterCloudProvider queryChildDocuments a isRawDocId 2");
            ...
            while (cursor.moveToNext()) {
                 Log.d("XULU_LOG","DecenterCloudProvider queryChildDocuments a isRawDocId 2-1");
                includeDownloadFromCursor(result, cursor, filePaths);
            }
            */
            Log.d("XULU_LOG","DecenterCloudProvider queryChildDocuments a isRawDocId 3");
    ...
    public Cursor queryRecentDocuments(String rootId, String[] projection)
            throws FileNotFoundException {
                   Log.d("XULU_LOG","DecenterCloudProvider queryRecentDocuments");
    ...
    public Cursor querySearchDocuments(String rootId, String query, String[] projection)
            throws FileNotFoundException {
        Log.d("XULU_LOG","DecenterCloudProvider querySearchDocuments");
    ...
    public ParcelFileDescriptor openDocument(String docId, String mode, CancellationSignal signal)
            throws FileNotFoundException {
        // Delegate to real provider
               Log.d("XULU_LOG","DecenterCloudProvider openDocument");
        final long token = Binder.clearCallingIdentity();
        try {
            if (RawDocumentsHelper.isRawDocId(docId)) {
    ...
    protected File getFileForDocId(String docId, boolean visible) throws FileNotFoundException {
        Log.d("XULU_LOG","DecenterCloudProvider getFileForDocId:" + docId);

        if (RawDocumentsHelper.isRawDocId(docId)) {
            Log.d("XULU_LOG","DecenterCloudProvider getFileForDocId:111" + docId);
            return new File(RawDocumentsHelper.getAbsoluteFilePath(docId));
        }
        Log.d("XULU_LOG","DecenterCloudProvider getFileForDocId: 1" + docId);

        if (DOC_ID_ROOT.equals(docId)) {
            return getDownloadsDirectory();
        }
        Log.d("XULU_LOG","DecenterCloudProvider getFileForDocId: 2" + docId);
    ...
    private void includeFilesFromSharedStorage(MatrixCursor result,
        ...
        for (File file : downloadsDir.listFiles()) {
            Log.d("XULU_LOG","DecenterCloudProvider" + file.getName());
    ...
    private static File getDownloadsDirectory() {
        return Environment.getExternalStoragePublicDirectory("Cloud");
    }
